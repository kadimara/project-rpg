...
export function createPlayer(spawnX: number, spawnY: number): Player {
  return {
    position: { tileX: spawnX, tileY: spawnY, px: spawnX * TILE, py: spawnY * TILE },
    movement: { dir: 'down', moving: false, moveStart: 0, moveDur: 260, fromX: 0, fromY: 0, toX: 0, toY: 0, path: [] },
    health: { hp: PLAYER_BASE_MAXHP, maxHp: PLAYER_BASE_MAXHP, flashUntil: 0, hpBarUntil: 0 },
    combat: { atkDamage: PLAYER_BASE_ATK, atkCooldown: PLAYER_BASE_ATK_COOLDOWN, lastAttack: 0 },
    attackTarget: null,
  };
}
