...
// player-facing walkability: terrain + no stepping onto any enemy tile, tree trunk, or npc
...
export function createWalkabilityPredicates(deps: WalkabilityDeps): WalkabilityPredicates {
  const { map, trees, player, enemies, isNpcAt } = deps;

  function walkable(x: number, y: number): boolean {
    return terrainWalkable(map, x, y) && !enemyAt(enemies, x, y) && !treeBlocksAt(trees, x, y) && !isNpcAt(x, y);
  }
  ...
  function bossFootprintWalkable(x: number, y: number, self: Enemy): boolean {
    for (const t of footprintAt(x, y, 2)) {
      ...
      if (isNpcAt(t.x, t.y)) return false;
      if (t.x === player.tileX && t.y === player.tileY) return false;
