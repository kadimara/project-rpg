...
export function updatePlayer(
  ...
  if (
    player.attackTarget &&
    targetStillValid &&
    player.attackTarget.health.hp > 0
  ) {
    ...
    for (const tile of footprint) {
      const d =
        Math.abs(player.position.tileX - tile.x) +
        Math.abs(player.position.tileY - tile.y);
      if (!nearest || d < nearest.d) nearest = { x: tile.x, y: tile.y, d };
    }
    const inRange = !!nearest && nearest.d === 1;
    if (inRange && nearest) {
      player.movement.path = [];
      player.movement.dir = dirBetween(
      ...
      if (player.movement.path.length === 0) {
        player.movement.path = bfsChase(
          deps.walkable,
          player.position.tileX,
          player.position.tileY,
          (x, y) => tileAdjacentToFootprint(x, y, footprint),
        );
      }
