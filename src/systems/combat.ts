...
import type { Combatant, Enemy, Player, Position } from '../types/entities.ts';
...
export function spawnFloatingText(state: CombatState, entity: { position: Position }, text: string, color: string, now: number): void {
  state.floatingTexts.push({ worldX: entity.position.px + TILE / 2, worldY: entity.position.py, text, color, born: now });
}

export function spawnDamageNumber(state: CombatState, entity: { position: Position }, dmg: number, color: string, now: number): void {
...
export interface CombatSystem {
  applyDamage: (attacker: Combatant, defender: Enemy | Player, dmg: number, now: number) => void;
  attemptAttack: (attacker: Combatant, defender: Enemy | Player, now: number) => void;
...
export function createCombatSystem(state: CombatState, enemies: Enemy[], player: Player, callbacks: CombatCallbacks): CombatSystem {
  ...
  function applyDamage(attacker: Combatant, defender: Enemy | Player, dmg: number, now: number): void {
  ...
  function attemptAttack(attacker: Combatant, defender: Enemy | Player, now: number): void {
