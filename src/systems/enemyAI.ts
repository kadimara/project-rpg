...
export function updateEnemies(
  ...
  for (const en of enemies) {
    if (
      en.isBoss &&
      en.aggro &&
      en.specialCooldown !== undefined &&
      now - (en.lastSpecial ?? 0) >= en.specialCooldown
    ) {
      en.lastSpecial = now;
      ...
      const nearX = Math.max(
        1,
        Math.min(MAP_W - 2, player.position.tileX + off[0]),
      );
      const nearY = Math.max(
        1,
        Math.min(MAP_H - 2, player.position.tileY + off[1]),
      );
      state.telegraphs.push({
        x: nearX,
        y: nearY,
    ...
    if (
      ...
      const isGoal = en.isBoss
        ...
        : (x: number, y: number) =>
            tileAdjacentToFootprint(x, y, [
              { x: player.position.tileX, y: player.position.tileY },
            ]);
      if (en.movement.path.length === 0) {
        en.movement.path = bfsChase(
          stepWalkable,
          en.position.tileX,
          en.position.tileY,
          isGoal,
      ...
      if (en.movement.path.length) {
        ...
        if (stepWalkable(next.x, next.y)) {
          startStep(
            ...
            next.x,
            next.y,
            dirBetween(en.position.tileX, en.position.tileY, next.x, next.y),
            now,
          );
        } else {
          en.movement.path = [];
