...
import { COLORS, OBSTACLE_COLOR, TREE_CANOPY_FILL, PLAYER_COLOR, ENEMY_COLOR, BOSS_COLOR, BARREL_COLOR } from './colors.ts';
...
export function renderWorldMap(worldCtx: CanvasRenderingContext2D, deps: WorldMapDeps): void {
  ...
  for (const b of barrels) {
    ...
    worldCtx.fillRect(b.position.tileX * WORLD_TILE, b.position.tileY * WORLD_TILE, WORLD_TILE, WORLD_TILE);
  }
  for (const en of enemies) {
    worldCtx.fillStyle = en.size === 2 ? BOSS_COLOR : ENEMY_COLOR;
    const span = en.size === 2 ? WORLD_TILE * 2 : WORLD_TILE;
    worldCtx.fillRect(en.position.tileX * WORLD_TILE, en.position.tileY * WORLD_TILE, span, span);
  }
  worldCtx.fillStyle = PLAYER_COLOR;
  worldCtx.fillRect(player.position.tileX * WORLD_TILE - 1, player.position.tileY * WORLD_TILE - 1, WORLD_TILE + 2, WORLD_TILE + 2);
}
